...
const AppHeader: React.FC<AppHeaderProps> = ({
  ...
  const userDropdownItems = [
    <>
      <DropdownItem key="local cluster 1 profile">http://localhost:9000</DropdownItem>
      <DropdownItem key="local cluster 1  user">http://localhost:8080</DropdownItem>
      <DropdownItem key="local cluster 32 logout">http://localhost:6000</DropdownItem>
    </>,
  ];
        ...
        <Toolbar id="toolbar" isFullHeight isStatic>
          <ToolbarContent>
            ...
            <ToolbarItem visibility={{ default: "hidden", md: "visible" }}>
              {!compactAvatar ? <UserAvatar /> : <UserAvatarCompact />}
            </ToolbarItem>
          </ToolbarContent>
        </Toolbar>
      </MastheadContent>
